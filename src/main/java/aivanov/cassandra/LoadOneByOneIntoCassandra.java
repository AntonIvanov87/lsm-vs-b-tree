...
class LoadOneByOneIntoCassandra {
  ...
  private static void insert(Edge edge, ResultMessage.Prepared preparedInsert) {
    var resp = new ExecuteMessage(
        preparedInsert.statementId,
        preparedInsert.resultMetadataId,
        QueryOptions.forInternalCalls(List.of(
            LongSerializer.instance.serialize(edge.sId),
            ByteSerializer.instance.serialize((byte) 0),
            LongSerializer.instance.serialize(edge.ut),

            LongSerializer.instance.serialize(edge.dId),
            ByteSerializer.instance.serialize((byte) 0),
            LongSerializer.instance.serialize(edge.ut),

            ByteSerializer.instance.serialize(edge.st),
            LongSerializer.instance.serialize(edge.ut),
            LongSerializer.instance.serialize(edge.pos),
            ByteSerializer.instance.serialize(edge.ann),
